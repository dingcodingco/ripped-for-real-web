...
export default function DownloadButtons() {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 justify-center">
      <motion.a
        href="#"
        className="flex items-center justify-center bg-white text-black px-8 py-4 rounded-xl hover:bg-gray-100 transition-all transform hover:scale-105 btn-transition relative overflow-hidden group"
        aria-label={t('appStore')}
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
      >
        <motion.div
          className="absolute inset-0 bg-gradient-to-r from-primary-orange/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"
          initial={{ x: '-100%' }}
          whileHover={{ x: '100%' }}
          transition={{ duration: 0.6 }}
        />
        <svg className="mr-3 relative z-10" style={{ width: '32px', height: '32px' }} viewBox="0 0 24 24" fill="currentColor">
